...
public class LeitorDePalestras {
	...
	private Palestra lerPalestra(Scanner scanner) {
		...
		TempoDeDuracao duracao = LIGHTING_STRING.contains(linha.substring(indiceDaDivisaoEntreNomeETempo + 1, linha.length() - 3)) 
				? TempoDeDuracao.LIGHTING 
				: new TempoDeDuracao(Integer.valueOf(linha.substring(indiceDaDivisaoEntreNomeETempo + 1, linha.length() - 3)));
